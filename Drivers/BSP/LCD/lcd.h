...
/******************************************************************************************/
/* LCD RST/BL/WR/RD/CS/RS/D0~D15 引脚 定义 */
...
#define LCD_BL_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 4; }while(0)   /* 所在IO口时钟使能 */

#define LCD_WR_GPIO_PORT                GPIOC
#define LCD_WR_GPIO_PIN                 SYS_GPIO_PIN7
#define LCD_WR_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 4; }while(0)   /* 所在IO口时钟使能 */

#define LCD_RD_GPIO_PORT                GPIOC
#define LCD_RD_GPIO_PIN                 SYS_GPIO_PIN6
#define LCD_RD_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 4; }while(0)   /* 所在IO口时钟使能 */

#define LCD_CS_GPIO_PORT                GPIOC
#define LCD_CS_GPIO_PIN                 SYS_GPIO_PIN9
#define LCD_CS_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 4; }while(0)   /* 所在IO口时钟使能 */

#define LCD_RS_GPIO_PORT                GPIOC
#define LCD_RS_GPIO_PIN                 SYS_GPIO_PIN8
#define LCD_RS_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 4; }while(0)   /* 所在IO口时钟使能 */

#define LCD_DATA_GPIO_PORT              GPIOB
#define LCD_DATA_GPIO_PIN               0XFFFF                                  /* 16个IO都用到 */
#define LCD_DATA_GPIO_CLK_ENABLE()      do{ RCC->APB2ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */

/******************************************************************************************/

/* LCD 端口控制函数 定义 */
//#define LCD_RST(x)      sys_gpio_pin_set(LCD_RST_GPIO_PORT,  LCD_RST_GPIO_PIN,  x)  /* 设置RST引脚 */
#define LCD_BL(x)       sys_gpio_pin_set(LCD_BL_GPIO_PORT, LCD_BL_GPIO_PIN, x)      /* 设置BL引脚 */
...
#define LCD_DATA_OUT(x) LCD_DATA_GPIO_PORT->ODR = x                                 /* 写D0~D15引脚 */
#define LCD_DATA_IN     LCD_DATA_GPIO_PORT->IDR                                     /* 读D0~D15引脚 */
